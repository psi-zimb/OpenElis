 ...
 */
public class TypeOfTestStatusDeleteAction extends BaseAction {
	...
	protected ActionForward performAction(ActionMapping mapping,
		...
		List typeOfTestStatuses = new ArrayList();

		for (int i = 0; i < selectedIDs.length; i++) {
			TypeOfTestStatus typeOfTestStatus = new TypeOfTestStatus();
			typeOfTestStatus.setId(selectedIDs[i]);
			typeOfTestStatus.setSysUserId(sysUserId);
			typeOfTestStatuses.add(typeOfTestStatus);
		}
		ActionMessages errors = null;
		try {
			TypeOfTestStatusDAO typeOfResultStatusDAO = new TypeOfTestStatusDAOImpl();
			typeOfResultStatusDAO.deleteData(typeOfTestStatuses);
			// initialize the form
			dynaForm.initialize(mapping);
		} catch (LIMSRuntimeException lre) {
    		//bugzilla 2154
			LogEvent.logError("TypeOfTestStatusDeleteAction","performAction()",lre.toString());
